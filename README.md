...
在 MuJoCo 3D 视窗里直接拖拽机械臂/夹爪是允许的，拖完会自动回到水平 home 位姿：

1. 相邻连杆互碰排除（内存碰撞过滤，不改模型文件）——凸包穿透不会再锁死关节
2. 触控盒/红色目标只与传感器侧碰撞——拖进臂内也不会砸翻机械臂
3. 姿态自动恢复——倾角 >20° 持续 2s 且无指令时自动复位场景（终端打印 `[自动恢复]`）
